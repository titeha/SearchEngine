...
Сервис готов к использованию (версия 1.0.0).

Реализовано:

- проверка работоспособности и готовности к поиску (`/health`, `/ready`), Docker healthcheck;
- информация о версии сервиса и библиотеки, активная конфигурация, допустимые параметры поиска;
- **несколько именованных индексов**, изолированных и строящихся параллельно без локов;
- построение in-memory индекса и построение из зарегистрированного источника данных;
- проверка запроса на построение индекса, получение состояния индексов и списка индексов;
- встроенные provider-ы источников данных: `in-memory`, `sqlite`, `postgres`, `firebird`, `sqlserver`, `mysql`, `oracle`;
- публичный API для подключения собственных reader-ов (в том числе экзотических СУБД, например IBM DB2);
- demo-сценарии SQLite (локально и в Docker), PostgreSQL и in-memory;
- точный, нечёткий и фонетический поиск, поиск внутри слова и поиск русских фамилий в латинской записи;
- ограничение количества документов и длины текста, snapshot индекса (сохранение, ручное и автоматическое восстановление);
- защита от перегрузки (лимит размера тела запроса, ограничение частоты запросов);
- опциональная аутентификация по API-ключу на мутирующих endpoint-ах;
- OpenAPI-описание API на `/openapi/v1.json`;
- высокая пропускная способность поиска: lock-free чтение по неизменяемому снимку индекса.
...
Сервис публикует машиночитаемое описание API в формате OpenAPI:

```http
GET {{host}}/openapi/v1.json
```

Документ описывает все HTTP-endpoint-ы и подходит для генерации клиентов и интеграций, в том числе для собственных СУБД-провайдеров. Endpoint открыт и не требует API-ключа.
...
Snapshot сохраняется атомарно: запись идёт во временный файл с последующим переименованием в целевой. Если процесс упадёт посреди записи, целевой snapshot останется целостным (прежним или новым), а не повреждённым.
...
| `Limits:RateLimit:PermitLimit` | число запросов от одного клиента за окно | `1000` |
...
Ограничение частоты — это guardrail против флуда с одного адреса, а не ограничение нормальной нагрузки: сам сервис на чтении выдерживает десятки тысяч запросов в секунду (см. раздел «Локальная нагрузочная smoke-проверка»). Для высоконагруженных сценариев лимит можно поднять, ограничивать на уровне шлюза или отключить (`IsEnabled: false`). Если сервис работает за обратным прокси, для корректного ограничения по IP нужно пробрасывать реальный адрес клиента (`X-Forwarded-For` и forwarded headers).

При построении очень больших индексов через `POST /v1/index` значение `MaxRequestBodyBytes` можно увеличить.

## Аутентификация (API-ключ)

Чтобы посторонние не могли перестроить или удалить индекс, мутирующие endpoint-ы можно защитить API-ключом. Защищаются запросы `POST /v1/index`, `POST /v1/index/from-source`, `POST /v1/index/restore`, `POST /v1/index/validate`. Проверки работоспособности (`/health`, `/ready`), состояние индекса, поиск и справочники остаются открытыми.

Проверка ключа **включена по умолчанию**, но применяется **только когда ключ задан**:

- ключ не задан — мутирующие endpoint-ы открыты; это рабочий сценарий для доверенного внутреннего контура (при старте сервис пишет предупреждение в лог);
- ключ задан — для мутирующих запросов требуется заголовок с этим ключом, иначе ответ `401`.
...
| Параметр | Описание | Значение по умолчанию |
|---|---|---|
| `Authentication:IsEnabled` | включить проверку API-ключа | `true` |
| `Authentication:ApiKey` | ожидаемый ключ; если пуст — проверка не применяется | `""` |
| `Authentication:HeaderName` | имя HTTP-заголовка с ключом | `X-Api-Key` |

Ключ следует задавать через секреты или переменные окружения, а не хранить в общем `appsettings.json`:
...
{
  "documents": [
    { "id": 1, "text": "Иванов Сергей Петрович" }
  ]
}
```

Ключ не возвращается наружу (в том числе в `GET /v1/config`). Сравнение ключа выполняется за постоянное время. Сервис работает по HTTP, поэтому в общем сегменте сети его следует размещать за обратным прокси с TLS.
...
- строит индекс из документов входящего JSON и из заранее настроенного источника данных через `POST /v1/index/from-source`;
- может сохранять snapshot индекса в файл и восстанавливать его вручную; автоматическое восстановление при старте доступно при включённом snapshot и `AutoRestoreOnStart` и сейчас охватывает только индекс по умолчанию;
- есть встроенные provider-ы `in-memory`, `sqlite`, `postgres`, `firebird`, `sqlserver`, `mysql` и `oracle`, а также публичный API для подключения собственных reader-ов;
- SQLite provider проверяется локально и через Docker demo-сценарий; SQL-provider-ы Firebird, SQL Server, MySQL/MariaDB и Oracle покрыты unit-тестами, а сквозное SQL-чтение — интеграционными тестами SQLite на общем базовом reader-е;
- не подключается к БД без заранее зарегистрированного reader-а; внешний API не принимает connection string и SQL-запрос;
- безопасность DB-доступа зависит от корректной настройки read-only пользователя БД;
- защищён от перегрузки (лимит тела запроса, ограничение частоты) и поддерживает опциональную аутентификацию по API-ключу на мутирующих endpoint-ах;
- работает по HTTP — в общем сегменте сети его следует размещать за обратным прокси с TLS;
- не отслеживает изменения в источнике данных (актуализация индекса — ручная или по расписанию вызывающей стороны);
- контейнер публикуется в GHCR по релизному тегу `service-v*`.
...
Smoke-тест нагружает сервис с одного адреса, то есть это ровно тот случай, против которого работает ограничение частоты запросов. Поэтому для замера «сырой» пропускной способности ограничение частоты на время прогона отключают:

```powershell
$env:SearchEngineService__Limits__RateLimit__IsEnabled = "false"
```

Пример вывода (Release, ограничение частоты отключено):
...
Чтение выполняется по неизменяемому снимку индекса без локов, поэтому поиск хорошо масштабируется под параллельной нагрузкой. Результаты зависят от машины, режима запуска, размера индекса и типа запросов и не являются гарантией производительности в production.
...
Сделано к версии 1.0.0: расширяемая валидация и общее чтение SQL-источников с таймаутами и лимитами; публичный API пользовательских reader-ов; provider-ы Firebird, SQL Server, MySQL/MariaDB и Oracle; несколько именованных индексов без локов; защита от перегрузки и аутентификация по API-ключу; OpenAPI-описание.

Сделано в версии 1.0.1: атомарное сохранение snapshot (запись во временный файл с переименованием) и единый ответ `ApiError` на необработанные исключения.

Дальнейшие направления:

1. довести устойчивость snapshot к сбоям питания (flush/fsync содержимого и каталога перед переименованием) и валидацию snapshot при загрузке;
2. автоматическое восстановление при старте для всех именованных индексов, а не только для индекса по умолчанию;
3. demo-сценарии и интеграционные тесты для Firebird, SQL Server, MySQL/MariaDB и Oracle с реальными БД;
4. актуализация индекса при изменении данных в источнике (см. раздел «Планируемая актуализация индекса»);
5. расширить наблюдаемость (метрики, структурные логи) и документацию по production-развёртыванию.
