...
Сейчас индекс строится только из списка документов, переданного во входящем JSON через `POST /v1/index`.

В будущем планируется добавить режим построения индекса из внешних источников данных.
...
3. добавить сохранение и восстановление snapshot индекса;
4. добавить построение индекса из заранее настроенного источника данных;
5. добавить безопасное подключение к БД с read-only доступом;
6. добавить ручную перестройку индекса по имени источника данных;
7. исследовать мониторинг изменений в БД для актуализации индекса;
8. добавить базовую защиту API;
9. добавить нагрузочные проверки сервиса;
10. добавить документацию по развёртыванию контейнера.
